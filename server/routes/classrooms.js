    ...
    */
    ...
    if (studentLimit) {
        if (studentLimit.includes(",")) {
            ...
            const limit = parseInt(studentLimit);
    ...
    const classrooms = await Classroom.findAll({
        attributes: [
            ...
            "name",
            "studentLimit",
            // Phase 9A: include average grade per classroom
            [
            ...
            [
                Sequelize.fn(
                    "COUNT",
                    Sequelize.col("StudentClassrooms.studentId")
                ),
                "numStudents",
            ],
        ],
...
router.get("/:id", async (req, res, next) => {
    ...
    classroom.studentCount = await classroom.countStudentClassrooms();
    classroom.overloaded = classroom.studentLimit < classroom.studentCount;
    const grades = await StudentClassroom.sum("grade", {
        where: {
            classroomId: req.params.id,
        },
    });
    classroom.avgGrade = grades / classroom.studentCount;
